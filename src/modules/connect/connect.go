...
const DefualtDisplayBufferFlushIntervalInMilliseconds = 200
...
func (c *Connect) OnBoradcast(msg []byte) {
	defer util.Recover()
	c.Lock.Lock()
	defer c.Lock.Unlock()
	if !c.ShowBroadcast {
	...
	c.WriteDisplayBuffer(NewTextBlock([]byte("\r\n" + app.System.Channel + strings.TrimPrefix(string(msg), "broadcast ") + "\r\n")).Bytes())
}
...
func (c *Connect) SetShowBroadcast(value bool) {
	c.Lock.Lock()
	defer c.Lock.Unlock()
	c.ShowBroadcast = value
}
func (c *Connect) flushUserinput() {
	...
	ui := strings.TrimRight(string(bs), "\n")
	if app.System.Command != "" && strings.HasPrefix(ui, app.System.Command) {
		switch ui {
		case app.System.Command + "1":
			go c.SetShowBroadcast(true)
			break
		case app.System.Command + "0":
			go c.SetShowBroadcast(false)
			break
		default:
			go Broadcast([]byte(strings.TrimPrefix(ui, app.System.Command)))
		}
